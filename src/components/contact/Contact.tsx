...
const defaultEmailTypeDataSource = [{ type: '@yonyou.com' }, { type: '@163.com' }, { type: '@126.com' }, { type: '@sina.com' }, { type: '@qq.com' }, { type: '@gmail.com' }, { type: '@yahoo.com' }, { type: '@hotmail.com' }]

interface ContactProps {
  label?: string
  singleLine?: boolean
  area?: boolean // 是否选择国际区号
  isSelectEmail?: boolean // 是否选择邮箱后缀
...
export default class Contact extends Component<ContactProps, ContactState> {
      ...
      case 'email': {
        ...
        const selectEmail = (
          <div style={{ display: 'flex', alignItems: 'center' }}>
            <Input textAlign={inputTextAlign} placeholder='name'/>
            <span className='yonui-contact-button' onClick={() => { this.onOpenModal() }}>{this.state.emailType}</span>
            <Icon size='xxs' type='down'/>
          </div>)
        const email = <Input textAlign={inputTextAlign} placeholder='name@yonyou.com'/>
      ...
      case 'telephone': {
        const areaContact = (
          <div style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <span style={{ background: '#E6E6E6', display: 'block', width: '1px', height: '14px', marginLeft: '4px' }}></span>
            <span style={{ color: '#555555', fontSize: '15px', paddingTop: '1px', marginLeft: '8px', marginRight: '4px' }}>{this.state.countryNum}</span>
            <Input type='tel' style={{ width: '140px' }} placeholder='-区号-座机号码'/>
          </div>)
        const contact = <Input type='tel' style={{ width: '140px' }} placeholder='座机号码-分机号码'/>
        return area ? areaContact : contact
      }
    }
  }
  ...
  render () {
    const { label, dataSource, singleLine, mode, area, isSelectEmail, emailDataSource } = this.props
    const { open } = this.state
    const content = this.getContent(mode, area, isSelectEmail)
    return (
